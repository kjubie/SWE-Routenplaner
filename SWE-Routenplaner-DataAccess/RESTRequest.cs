...
namespace SWEN2_Tourplanner_DataAccess
{
    public class RESTRequest : IQuery
    {
        ...
        private string _url = "https://localhost:7221";

        public async Task<Tours> GetTours()
        {
            using var client = new HttpClient();
            var response = await client.GetStringAsync(_url  + "/api/Tour");
        ...
        {
            using var client = new HttpClient();
            var response = await client.GetStringAsync(_url + "/api/Tour/" + tourname);
        ...
        {
            using var client = new HttpClient();
            var response = await client.GetStringAsync(_url + "/api/Tour/image/" + tourname);
        ...
        {
            string url = _url + "/api/Tour/" + nameTourToDelete;
            using var client = new HttpClient();
